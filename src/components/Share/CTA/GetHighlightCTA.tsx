...
export default function GetHighlightCTA() {
      ...
      return (
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <button className="duration-250 flex-shrink-0 whitespace-nowrap rounded-[8px] bg-teal px-3 py-1.5 text-xs font-bold text-black transition-all ease-in-out hover:bg-primary-60">
              Get Highlight
            </button>
          </DropdownMenuTrigger>
  ...
  return (
    <div className="fixed bottom-4 w-full max-w-[712px] px-8 md:px-4">
      <div className="flex items-center justify-between rounded-[20px] border border-light-10 bg-secondary p-3">
        ...
        <p className="font-base mx-3 flex-grow truncate text-center text-[14px] text-tertiary md:text-[16px]">
          {isMobile ? (
            <span>Learn more about Highlight</span>
          ) : (
            <>
              <span className="md:hidden">Highlight is your personal AI</span>
              <span className="hidden md:inline">Highlight is your personal AI for getting answers and doing work</span>
            </>
          )}
        </p>
        {renderButton()}
