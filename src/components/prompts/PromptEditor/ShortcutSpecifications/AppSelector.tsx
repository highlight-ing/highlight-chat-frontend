...
export default function AppSelector({ shortcutName }: { shortcutName: string }) {
  ...
  }, [appVisibility, saveShortcutPreferences, selectedApp])
  ...
  const onUpdateSelectedApp = (value: AppAvailability) => {
  ...
  const contextMenuItems = useMemo(() => {
    const items: MenuItemType[] = menuOptions.map((option) => ({
      ...
      onClick: () => onUpdateSelectedApp(option.value), // Uses store's setSelectedApp
    }))
