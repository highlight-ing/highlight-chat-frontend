...
export const useSubmitQuery = () => {
  ...
  const { getOrCreateConversationId, resetConversationId } =
  ...
  const handleSubmit = async () => {
    ...
    if (query) {
      ...
      const { screenshot, audio, fileTitle } = addAttachmentsToFormData(
        formData,
        attachments
      );
      ...
      let contextString =
        conversationHistory ??
        "This is a new conversation with Highlight Chat.";
