...
export async function fetchPromptText(slug: string) {
  const { data: prompt, error } = await supabaseAdmin.from('prompts').select('*').eq('slug', slug).maybeSingle()
...
export async function fetchPrompts(authToken: string) {
  ...
  const { data: prompts, error } = await supabaseAdmin.from('prompts').select('*')
...
export async function deletePrompt(slug: string, authToken: string) {
  ...
  const { error } = await supabaseAdmin.from('prompts').delete().eq('slug', slug).eq('user_id', userId)
